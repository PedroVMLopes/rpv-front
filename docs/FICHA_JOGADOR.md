...
- **Magia** é o grimório de sessão (lista por nível + slots + preparar). Escondida quando `characterHasMagic` é falso. Preparação na ficha grava `choices.preparedSpells` para classes `spellbook` / `prepared-list`. Combate continua listando magias castáveis na economia de ação.

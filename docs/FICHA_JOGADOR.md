...
| **Combate** | Economia de ação | Ataques, ações/bônus/reações, magias castáveis, CD/ataque de magia, recursos de classe, defesa e resistências — agrupados por **tipo de ação**, não por origem (classe vs item) | [`CombatTab`](../apps/web/components/characters/PlayerSheet/tabs/CombatTab.tsx), [`CastingStatsPanel`](../apps/web/components/characters/PlayerSheet/combat/CastingStatsPanel.tsx), [`ClassResourcesPanel`](../apps/web/components/characters/PlayerSheet/combat/ClassResourcesPanel.tsx), [`DefenseSavesPanel`](../apps/web/components/characters/PlayerSheet/combat/DefenseSavesPanel.tsx) |
| **Magia** | Grimório de sessão | CD/ataque, slots (spell/pact), lista por nível (em `spellbook`: conhecidas não preparadas atenuadas), preparar magias (`preparedSpells`) para modos `spellbook` / `prepared-list`. Aba **escondida** se o personagem não tem magia (`characterHasMagic`) | [`MagicTab`](../apps/web/components/characters/PlayerSheet/tabs/MagicTab.tsx), [`MagicSpellbookPanel`](../apps/web/components/characters/PlayerSheet/magic/MagicSpellbookPanel.tsx), [`PrepareSpellsModal`](../apps/web/components/characters/PlayerSheet/magic/PrepareSpellsModal.tsx), [`SpellSlotsPanel`](../apps/web/components/characters/PlayerSheet/magic/SpellSlotsPanel.tsx) |
| **Mochila** | Posse e equipamento | Três painéis (Equipamento → Posses → Cosmético) + resumo moeda/peso; filtros só em Posses — ver [`docs/INVENTORY.md`](INVENTORY.md). Personagem recém-criado: loot inicial aparece em Posses ou Equipamento conforme policy (`carried` vs equipável stowed); **sem grants de item** até equipar na ficha | [`InventoryTab`](../apps/web/components/characters/PlayerSheet/tabs/InventoryTab.tsx) (ficha); [`CharacterCardInventory`](../apps/web/components/characters/CharacterCard/CharacterCardInventory.tsx) (carousel legado) |
...
- **Magia** é o grimório de sessão (lista por nível + slots + preparar). Escondida quando `characterHasMagic` é falso. Preparação na ficha grava `choices.preparedSpells` para classes `spellbook` / `prepared-list`. Em modo `spellbook`, magias conhecidas não preparadas aparecem atenuadas (só leitura); Combate continua listando apenas magias castáveis na economia de ação.
...
| Magia | Grimório por nível + preparação na ficha; aba escondida sem magia |
